...
import { FileText, Clock, Eye, Rss, TrendingUp, AlertCircle } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const { data: topArticles } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('articles')
        .select('id, title, view_count, source:sources(name)')
        .eq('status', 'published')
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoading ? '...' : stats?.totalArticles.toLocaleString()}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-amber-700 dark:text-amber-400">
                {isLoading ? '...' : stats?.pendingArticles.toLocaleString()}
              </div>
